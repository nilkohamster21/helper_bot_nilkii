...
async def new_presentation(message, context):
    ...
    await message.reply_media_group(media=media_group)  # Высылаются изображения - шаблоны для презентаций
...
async def more_templates(update, context):
    ...
    if template_response == 'ещё шаблоны':
        ...
        IMAGE_LINKS2 = [  # Ссылки на изображения чтобы не скачивать изображения
